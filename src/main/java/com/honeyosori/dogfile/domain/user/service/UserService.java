...
public class UserService {
    ...
    @Autowired
    public UserService(UserRepository userRepository, JwtUtility jwtUtility, RedisTemplate<String, String> redisTemplate, DogusClient dogusClient, DogclubClient dogclubClient, KlatComponent klatcomponent, RestClient restClient) {
    ...
    public CreateUserDto register(CreateUserDto createUserDto) {
        ...
        this.userRepository.findUserByEmail(email).ifPresent(u -> {
            if (u.getDeletedAt() == null) {
                throw new OAuthException(CommonResponse.EMAIL_EXISTS);
            } else {
                throw new OAuthException(CommonResponse.WITHDRAWN);
            }
        });
        ...
        CreateDogusUserDto createDogusUserDto = CreateDogusUserDto.builder().dogfileUserId(newUser.getId()).accountName(createUserDto.accountName()).profileImageUrl(profileImageUrl).build();

        CreateDogclubUserDto createDogclubUserDto = CreateDogclubUserDto.builder().dogfileUserId(newUser.getId()).accountName(createUserDto.accountName()).profileImageUrl(profileImageUrl).build();

        CreateKlatUserDto createKlatUserDto = CreateKlatUserDto.builder().userId(newUser.getId()).password(newUser.getId()).username(createUserDto.accountName()).profileImageUrl(profileImageUrl).build();
        ...
        try {
            ...
            throw new OAuthException(CommonResponse.INTERNAL_SERVER_ERROR, "DOGCLUB 등록 실패");
        }

        try {
            log.info("[KLAT] Send User Create Request");
            createKlatUser(createKlatUserDto);
        } catch (Exception e) {
    ...
    public UpdateUserDto updateUser(UpdateUserDto updateUserDto, String email) {
        User user = this.userRepository.findUserByEmail(email).orElseThrow(() -> new GlobalException(CommonResponse.USER_NOT_FOUND));
        ...
        if (updateUserDto.accountName() != null || updateUserDto.profileImageUrl() != null) {
            UpdateDogusUserDto updateDogusUserDto = UpdateDogusUserDto.builder().accountName(updateUserDto.accountName()).profileImageUrl(updateUserDto.profileImageUrl()).build();

            UpdateDogclubUserDto updateDogclubUserDto = UpdateDogclubUserDto.builder().accountName(updateUserDto.accountName()).profileImageUrl(updateUserDto.profileImageUrl()).build();
            ...
            try {
                ...
                UpdateDogusUserDto undoUpdateUserDto = UpdateDogusUserDto.builder().accountName(oldAccountName).profileImageUrl(oldProfileImageUrl).build();
    ...
    public ResponseEntity<?> login(LoginDto loginDto) {
        ...
        User user = this.userRepository.findUserByEmailAndDeletedAtIsNull(email).orElseThrow(() -> new GlobalException(CommonResponse.USER_NOT_FOUND));
        ...
        try {
            log.info("[KLAT] Send User Create Request");
            KlatLoginDto klatLoginDto = KlatLoginDto.builder().userId(user.getId()).password(user.getId()).build();

            KlatResponseDto klatResponse = loginToKlat(klatLoginDto);
            klatLoginToken = klatResponse.loginToken();
        } catch (Exception e) {
            throw new RuntimeException("KLAT 등록 실패", e);
        }
        ...
        throw new GlobalException(CommonResponse.WRONG_PASSWORD);
    }

    private void createKlatUser(CreateKlatUserDto createKlatUserDto) {
        restClient.post().uri("https://api.talkplus.io/v1.4/api/users/create").headers(headers -> {
            headers.set("app-id", klatComponent.getKLAT_APP_ID());
            headers.set("api-key", klatComponent.getKLAT_API_KEY());
        }).body(createKlatUserDto).retrieve().body(KlatResponseDto.class);
    }

    private KlatResponseDto loginToKlat(KlatLoginDto klatLoginDto) {
        try {
            return restClient.post().uri("https://api.talkplus.io/v1.4/api/users/login").headers(headers -> {
                headers.set("app-id", klatComponent.getKLAT_APP_ID());
                headers.set("api-key", klatComponent.getKLAT_API_KEY());
            }).body(klatLoginDto).retrieve().body(KlatResponseDto.class);
        } catch(Exception e) {
            throw new GlobalException(CommonResponse.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<?> refresh(String email, String refreshToken) {
        ...
        if (Objects.equals(whitelistToken, "")) {
            throw new GlobalException(CommonResponse.UNAUTHENTICATED);
        } else if (!Objects.equals(whitelistToken, refreshToken)) {
            throw new GlobalException(CommonResponse.EXPIRED_JWT_TOKEN);
        }
    ...
    public CommonResponse deleteUser(String email) {
        User user = this.userRepository.getUserByEmailAndDeletedAtIsNull(email).orElseThrow(() -> new GlobalException(CommonResponse.USER_NOT_FOUND));
    ...
    @Transactional
    public CommonResponse cancelDeletion(String email) {
        User user = this.userRepository.getUserByEmailAndDeletedAtIsNotNull(email).orElseThrow(() -> new GlobalException(CommonResponse.USER_NOT_FOUND));
    ...
    public UserInfoDto findUserById(String id) {
        User user = this.userRepository.findByIdAndDeletedAtIsNull(id).orElseThrow(() -> new GlobalException(CommonResponse.USER_NOT_FOUND));

        return UserInfoDto.of(user);
    }

    public UserInfoDto findUserByAccountName(String accountName) {
        User user = this.userRepository.findByAccountNameAndDeletedAtIsNull(accountName).orElseThrow(() -> new GlobalException(CommonResponse.USER_NOT_FOUND));
    ...
    public UserInfoDto findUserByPhoneNumber(String phoneNumber) {
        User user = this.userRepository.findByPhoneNumberAndDeletedAtIsNull(phoneNumber).orElseThrow(() -> new GlobalException(CommonResponse.USER_NOT_FOUND));

        return UserInfoDto.of(user);
    }

    public KlatResponseDto getKlatInfo(String email) {
        User user = this.userRepository.findUserByEmail(email).orElseThrow(() -> new GlobalException(CommonResponse.USER_NOT_FOUND));
        ...
        return loginToKlat(klatLoginDto);
    }
}
